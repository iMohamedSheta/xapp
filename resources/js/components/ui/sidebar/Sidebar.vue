...
import type { SidebarProps as SidebarPropsType } from '.'
...
const props = defineProps<Partial<SidebarPropsType>>()

const side = computed(() => sidebarProps.value.side)
const variant = computed(() => sidebarProps.value.variant)
const collapsible = computed(() => sidebarProps.value.collapsible)
const classes = computed(() => sidebarProps.value.class)
...
const loadFromStorage = () => {
  try {
    ...
    if (saved) {
      const sidebarState = JSON.parse(saved)
      if (setOpen && typeof sidebarState.open === 'boolean') {
        setOpen(sidebarState.open)
      }
      if (typeof sidebarState.openMobile === 'boolean') {
        setOpenMobile(sidebarState.openMobile)
      }
    }
  } catch (error) {
